require(testthat)
context("metric_type_recall function")
...
test_that("the metric_type_recall function works", {
  ...
  count_attr <- "count"
  result <- metric_type_recall(experiment, short = TRUE, count_attr = count_attr)
  ...
  # Note that the metric is calculated only for the types in the corruption,
  # despite the fact that the results contain other types (namely, "shift").
