...
test_that("the ddiff function works", {
  ...
  result <- ddiff(df1, df2 = df2, patch_generators = list(gen_patch_transform),
                  patch_penalties = 0.6, break_penalty = 0.99, permute_penalty = 0.4)
  ...
  result <- ddiff(df1, df2 = df2, patch_generators = list(gen_patch_transform),
                  ...
                  permute_penalty = 0.4, as.list = TRUE)
  ...
  # This time an erroneous permutation is included.
  expect_equal(length(result), expected = 3)
  expect_equal(patch_type(result[[1]]), expected = "scale")
  expect_equal(patch_type(result[[2]]), expected = "shift")
  expect_equal(patch_type(result[[3]]), expected = "perm")
  ...
  result <- ddiff(df1, df2 = df2, patch_generators = list(gen_patch_transform),
                  patch_penalties = 0.4, break_penalty = 0.99, permute_penalty = 0.4)
