...
#'  frame, and
#'  \item after coersion to type \code{character}, all of the values (or at
#'  least one of them, if \code{strict = FALSE}) in each of the relevant
#'  columns of the data frame are contained in the set of element names in the
#'  \code{datadiff:::ENCODING} element of the \code{params} object.
#' }
#'
#' @param obj
#' A \code{patch} object.
...
#' A data frame.
#' @param strict
#' A logical flag used to specify whether the encoding in the patch
#' \code{params} object must include all of the values in the relevant columns
#' (\code{strict = TRUE}), or merely at least one of those values
#' (\code{strict = FALSE}). The default value is \code{TRUE}
#' @param ...
...
is_compatible.encode.patch <- function(obj, df, strict = TRUE, ...) {
  ...
  # Check that the encoding is effectual. If strict is TRUE (respectively
  # FALSE) then all (respectively at least one) of the values in the relevant
  # columns must exist in the element names of the encode_factor.
...
#' @param convert_type
#' A logical flag. If \code{TRUE} (the default) then the \code{type.convert}
...
return_value.encode.patch <- function(obj, df, strict = TRUE,
  ...
  # Apply the encoding in the factor, using type.convert to recover the type
  # (with the as.is argument set so that, in the case of character encodings,
  # each new column is a factor if and only if the original column was).
...
#' @param encode_factor
#' A factor (or vector) with named elements specifying a re-encoding of
#' categorical data.
#'
#' @seealso \code{\link{patch}}
#'
#' @export
encode_patch_params <- function(column_indices = integer(),
                                encode_factor = factor()) {
