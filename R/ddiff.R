...
#' @param break_penalty
#' The penalty associated with a break patch. By default no scaling is applied
...
#' @param ignore_cols
#' (Optional) An integer vector specifying corresponding columns in \code{df1}
...
ddiff <- function(df1, df2,
  ...
  stopifnot(permute_penalty >= 0)
  stopifnot(length(patch_generators) == length(patch_penalties))
  stopifnot(all(patch_penalties >= 0))
  stopifnot(break_penalty >= 0)
  if (!is.null(ignore_cols)) {
    stopifnot(is.integer(ignore_cols))
    stopifnot(all(ignore_cols %in% 1:min(ncol(df1), ncol(df2))))
    ignore_cols <- sort(ignore_cols)
    df1 <- df1[-ignore_cols]
    df2 <- df2[-ignore_cols]
  }
  stopifnot(ncol(df2) > 1)
  ...
  cw_candidates <- columnwise_candidates(df1, df2 = df2,
                                         ...
                                         scale_break_penalty = scale_break_penalty,
                                         verbose = verbose)
  ...
  if (!identical(perm, 1:ncol(df2)))
    patch_list <- c(patch_list, patch_permute(perm))

  # Take any ignored columns into account.
  if (!is.null(ignore_cols)) {
    sink <- sapply(patch_list, FUN = function(p) {
      admit_columns(p, cols = ignore_cols)
    })
  }
