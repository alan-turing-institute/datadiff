#' Generate candidate patches by pairwise comparison
#'
#' @description
#' TODO.
#'
...
pairwise_candidates <- function(df1, df2,
  ...
  purrr::map(1:ncol(df1), .f = function(i) {
    purrr::map(1:ncol(df2), .f = function(j) {
      ...
      sink <- sapply(1:length(patch_generators), FUN = function(i) {
        if (!is.null(patches[[i + 1]]))
          attr(patches[[i + 1]], "penalty") <<-
            scale_penalty(patch_penalties[i], nx = nrow(df1), ny = nrow(df2))
      })
      ...
      if (verbose) {
        cat(paste0("[", i, ", ", j, "] pairwise candidate mismatches & penalties:\n"))
        cat(mismatches, sep = "\t", "\n")
        cat(penalties, sep = "\t", "\n")
      }
